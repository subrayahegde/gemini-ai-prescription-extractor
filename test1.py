...
def main():
    st.title('Medical Prescription Parsing (Gemini Version)')
    global parser
    parser = JsonOutputParser(pydantic_object=PrescriptionInformations)
    ...
    if uploaded_file is not None:
        ...
        with st.spinner('Processing Prescription...'):
            ...
            if 'medications' in final_result and final_result['medications']:
                ...
                for i, med in enumerate(flat_meds):
                    med_name = med.get('Medication name') or med.get('name', f'Medication {i+1}')
                    st.markdown(f"### 💊 {med_name}")

                    st.markdown(f"""
                    - **Dosage:** {med.get('Dosage') or med.get('dosage', '')}
                    - **Frequency:** {med.get('Frequency') or med.get('frequency', '')}
                    - **Duration:** {med.get('Duration') or med.get('duration', '')}
                    - **Instructions:** {med.get('Instructions') or med.get('instructions', '')}
                    """)

                    alt_key = f"show_alt_{i}"
                    if alt_key not in st.session_state:
                        st.session_state[alt_key] = False

                    if st.button(f"Show Alternative for {med_name}", key=f"alt_button_{i}"):
                        st.session_state[alt_key] = True

                    if st.session_state[alt_key]:
                        with st.spinner("Finding alternatives..."):
                            alt_prompt = f"""
                            Suggest an alternative medicine for:
                            Medication: {med_name}
                            Dosage: {med.get('Dosage') or med.get('dosage', '')}
                            Frequency: {med.get('Frequency') or med.get('frequency', '')}
                            Duration: {med.get('Duration') or med.get('duration', '')}
                            ...
                            - Note on whether doctor consultation is needed
                            """

                            model = genai.GenerativeModel('gemini-1.5-flash')
                            alt_response = model.generate_content(alt_prompt)

                        with st.expander("🔄 Suggested Alternative"):
