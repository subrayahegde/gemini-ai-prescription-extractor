        ...
        with st.spinner('Processing Prescription...'):
            ...
            if 'medications' in final_result and final_result['medications']:
                ...
                for i, med in enumerate(flat_meds):
                    with st.expander(f"💊 {med.get('Medication name', med.get('name', 'Medication'))}"):
                        st.markdown(f"""
                        ...
                        if st.button(f"Show Alternative for {med.get('Medication name', med.get('name', f'Med {i+1}'))}", key=f"alt_button_{i}"):
                            with st.spinner("Finding alternatives..."):
                                alt_prompt = f"""
                                Suggest an alternative medicine for:
                                Medication: {med.get('Medication name', med.get('name', ''))}
                                Dosage: {med.get('Dosage', '')}
                                Frequency: {med.get('Frequency', '')}
                                Duration: {med.get('Duration', '')}
                                Instructions: {med.get('Instructions', '')}
